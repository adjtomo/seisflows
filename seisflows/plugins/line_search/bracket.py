...
class Bracket(Base):
    ...
    def calculate_step(self):
        ...
        # If misfit is reduced and then increased, we've bracketed. Pass 
            ...
            print "\tBracket okay and step length reasonable, pass." 
            alpha = x[f.argmin()]
            status = 1

        # if misfit is reduced but not close, set to quadratic fit
        elif _check_bracket(x,f):
            print "\tBracket okay but step length unreasonable, manual step..."
            alpha = polyfit2(x,f)
            status = 0
        ...
        # step_count_max exceeded, fail
            ...
            print "\t step_count_max exceeded. Fail"
            alpha = None
            status = -1
        ...
        # stop because safeguard prevents us from going further
            ...
            print "step_len_max exceeded, pass"
