    ...
    ***
    ...
    def evaluate_initial_misfit(self, path_model=None, save_residuals=None,
        ...
        if self.iteration == 1:
            ...
            # Exposing initial model to the optimization library. Ensure that 
            # we only take the parameters and regions that will be updated
            dst = self.optimize.path._m_new
            unix.mkdir(dst)
            for src in Model(self.path.model_init, 
            ...
            # Non-Thrifty, run forward simulation with previous model.
                ...
                # Exposing current model to the optimization library 
                dst = self.optimize.path._m_new
                shutil.rmtree(dst)
                unix.mkdir(dst)
                for src in Model(self.path._model_grad).filenames:
                    unix.cp(src, dst) 
    ...
    def evaluate_gradient_from_kernels(self):
        ...
        # Exposing gradient to the optimization library as symlink
        dst = self.optimize.path._g_new
        unix.mkdir(dst)
        for src in Model(self.path._gradient).filenames:
            unix.cp(src, dst)
    ...
    def finalize_iteration(self):
        ...
        if self.export_model:
            ...
            m_new.apply(actions=[], values=[], export_to=path_m)
