...
class Inversion(Migration):
    ...
    ***
    ...
    def evaluate_initial_misfit(self, sum_residuals=True, **kwargs):
        """
        ...
        :type sum_residuals: bool
        :param sum_residuals: Used to sum all residuals files from preprocessing
            module into a single value (f_new) which is used for tracking the
            gradient of the misfit function. Defaults to True which is normally
            fine for all purposes. This is made an option because some workflows
            like the ambient noise inversion require multiple forward runs
            before summing residuals, so it doesn't make sense to sum each time
            this function is called.
        """
        # Location to save 'residuals_*.txt files which are used to calculate
        # total misfit (f_new). Allow inheriting workflows to override this
        # path if more specific file naming is required. Hard coding the step
        # count (last value) as 0 because this is only ever run for step==0
        save_residuals = kwargs.get(
            "save_residuals",
            os.path.join(self.path.eval_grad,
        ...
        if self.iteration == 1:
            ...
            # changed. If they have (not-thrifty), then we need to rerun the
            # fwd solver
            if self.thrifty and (
                ...
                # Run forward simulation with previous model. Hard set line
                # search step count in residual file names to 0 since it is
                # assumed we are running the initial misfit evaluation (i??s00)
                self.system.run(
                    [self.run_forward_simulations,
                     self.evaluate_objective_function],
                    path_model=path_model, save_residuals=save_residuals
                )
        if sum_residuals:
            # Read in all avail residual files generated by preprocessing module
            residuals_files = save_residuals.format(src="*")
