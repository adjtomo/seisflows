...
class specfem2d(object):
    """ Python interface for SPECFEM2D
      ...
      load, save
        ...
        models and kernels are stored as text files, and in memory, as
    ...
    def prepare_data(self, data_path=None, **kwargs):
        """ Prepares data for inversion or migration
          ...
          SPECFEM2D expects all components to exist, even ones not actually in
          use for the inversion.
        """
        unix.cd(self.path)
    ...
    def apply_hess(self, path='', hessian='Newton'):
        ...
        preprocess.prepare_apply_hess(unix.pwd())
