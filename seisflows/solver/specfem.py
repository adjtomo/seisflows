...
class Specfem:
    ...
    ***
    ...
    def kernel_wildcard(self, par="*"):
        """
        Returns a wildcard identifier to search for kernels generated by an 
        adjoint simulation. An example SPECFEM2D/3D kernel filename (in 
        FORTRAN binary file format) is: 'proc000001_rho_kernel.bin'

        Allows dynamically searching for specific kernels when renaming, moving
        ...
        :return: wildcard identifier for channels
        """
        return f"proc??????_{par}_kernel{self._ext}"
    ...
    def adjoint_simulation(self, executables=None, save_kernels=False,
        ...
        for tag in ["alpha", "alpha[hv]"]:
            names = glob(self.kernel_wildcard(par=tag))
            if names:
        ...
        if export_kernels:
            ...
            for par in self._parameters:
                unix.cp(src=glob(self.kernel_wildcard(par=par)),
                        dst=export_kernels)

        if save_kernels:
            unix.mkdir(save_kernels)
            for par in self._parameters:
                unix.mv(src=glob(self.kernel_wildcard(par=par)),
                        dst=save_kernels)
